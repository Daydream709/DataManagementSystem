...
def restore_bank_version(owner_id: str, item_id: str, target_version_item_id: str) -> dict[str, Any]:
    ...
    if str(target["_id"]) == str(current["_id"]) and current.get("is_latest"):
        raise ValidationError("目标版本已经是当前最新版本")

    # 将同链所有版本设为非最新
    bank.update_many(
        {"chain_id": current["chain_id"], "owner_id": owner_oid},
        {"$set": {"is_latest": False}},
    )
    # 将目标版本设为最新
    bank.update_one(

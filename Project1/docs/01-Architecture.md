...
6. 统计模块：按题型聚合统计。
7. 【第二阶段新增】题库模块：题目保存、版本链管理、共享（定向分享）、使用追踪、跨问卷统计。
...
3. 用户切换到旧版本时，直接将目标版本的 is_latest 设为 true，当前最新版本的 is_latest 设为 false，不创建新版本。可在已有版本之间自由切换。
...
共享机制在版本链级别生效：共享操作将目标用户 ID 写入链上所有版本的 shared_with 数组，新版本自动继承。题目仅支持定向分享给指定用户，不支持公开分享。

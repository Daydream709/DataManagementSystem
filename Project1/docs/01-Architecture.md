...
6. 统计模块：按题型聚合统计。
7. 【第二阶段新增】题库模块：题目保存、版本链管理、共享、公开、使用追踪、跨问卷统计。
...
3. 用户恢复旧版本时，同样创建新记录（version 继续递增），内容为旧版本数据，保证历史不丢失。
...
共享机制在版本链级别生效：共享操作将目标用户 ID 写入链上所有版本的 shared_with 数组，新版本自动继承。公开标志 is_public 同理。

  ...
  <div class="min-h-screen">
    <header class="sticky top-0 z-20 border-b border-slate-200/80 bg-white/80 backdrop-blur">
      <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-6">
        <div>
          <h1 class="font-display text-xl font-extrabold tracking-tight text-ink">问卷系统 · Phase 1</h1>
          <p class="text-xs text-slate-500">Vue3 + Django + MongoDB</p>
        </div>
        <div class="flex items-center gap-3">
          <span class="hidden text-sm text-slate-600 md:block">{{ authStore.user?.username }}</span>
